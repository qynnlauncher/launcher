...
import com.hg.qynnlauncher.api2.server.BetterMimeTypeMap
import com.hg.qynnlauncher.api2.server.HTTPStatusCode
import com.hg.qynnlauncher.api2.server.errorResponse
...
private const val TAG = "QYNNFileServer"

class QYNNFileServer(
...
)
{
    ...
    suspend fun handle(req: WebResourceRequest): WebResourceResponse
    {
        if (CurrentAndroidVersion.supportsScopedStorage() && !Environment.isExternalStorageManager())
            return errorResponse(HTTPStatusCode.ServiceUnavailable, "QYNN does not have the \"Manage All Files\" permission.")
        ...
        if (!canonicalFile.canRead())
            return errorResponse(HTTPStatusCode.Forbidden, "QYNN does not have permission to read the requested file.")
